...
- **Optimize images:** `npm run optimize-images` (sharp; mozjpeg/png; max width 2400, JPEG q88). Cached in `.cache/` by source hash; `turbo run optimize-images` restores that cache across Vercel deploys via Remote Cache. Runs automatically in Vercel `buildCommand` and `npm run deploy`.
...
- Set `IP_HASH_SECRET` in Convex too (same value as Vercel): `npx convex env set IP_HASH_SECRET "$IP_HASH_SECRET"` — required for mutation ipProof verification.
